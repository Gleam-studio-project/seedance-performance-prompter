...
- 已具备可选 Basic Auth、Docker 和 Vercel 部署入口；提交 `ec9a7f0` 已发布到 Vercel Production，现有线上版本仍按旧配置要求登录。
...
- 本地新规则尚未部署到 Vercel Production；本次已将登录改为显式 `ENABLE_AUTH=true` 才启用，生产发布仍需单独部署。
